...
class Upload {
    ...
    private function setDriver($driver = null, $config = null){
        ...
        if(!$this->uploader){
//            E("不存在上传驱动：{$name}");
            E("No upload driver does not exist:{$name}");
        }
    }
...
//            $this->error = '未知上传错误！';
        ...
        if (!is_uploaded_file($file['tmp_name'])) {
            //$this->error = '非法上传文件！';
            $this->error = 'Illegally uploading files';
            return false;
        }
...
//            $this->error = '上传文件大小不符！';
            $this->error = 'The size of the uploaded file does not conform to the size of the file';
            return false;
        }
        ...
        if (!$this->checkMime($file['type'])) {
            ...
            $this->error = 'Uploading file MIME type is not allowed';
            return false;
        }
        ...
        if (!$this->checkExt($file['ext'])) {
            ...
            $this->error = 'Uploading file suffix is not allowed';
            return false;
        }
...
//                $this->error = '没有文件被上传！';
            ...
            case 6:
//                $this->error = '找不到临时文件夹！';
                $this->error = 'Unable to find a temporary folder';
                break;
            case 7:
//                $this->error = '文件写入失败！';
